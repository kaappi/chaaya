...
#define CH_IR_BOUND_MAX 256

typedef struct ChLowerScope {
    const char *names[CH_IR_BOUND_MAX];
    int count;
    struct ChLowerScope *parent;
} ChLowerScope;
...
static bool is_hygienic_name(const char *name) {
    return name && strncmp(name, "__hyg_", 6) == 0;
}

static bool scope_binds(const ChLowerScope *scope, const char *name) {
    for (const ChLowerScope *s = scope; s; s = s->parent) {
...
/* A head shadows a keyword/prim only when it is a plain (non-__hyg_) name
 * that is bound in an enclosing lambda parameter list. */
static bool head_is_shadowed(ChValue head, const ChLowerScope *scope) {
    if (!ch_is_symbol(head) || !scope) {
        return false;
    }
    ChSymbol *sym = ch_as_symbol(head);
    if (is_hygienic_name(sym->name)) {
        return false;
    }
    return scope_binds(scope, sym->name);
}
...
static ChCompileStatus lower_list_nodes(ChCompiler *c, ChValue list, ChIrNode ***out_items,
                                        size_t *out_count, ChLowerScope *scope) {
    size_t count = 0;
    ...
    for (ChValue it = list; ch_is_pair(it); it = ch_cdr(it), i++) {
        if (lower_node(c, ch_car(it), &items[i], scope) != CH_COMPILE_OK) {
...
static ChCompileStatus lower_if(ChCompiler *c, ChValue expr, ChValue args, ChIrNode **out,
    ...
    if (lower_node(c, test, &test_node, scope) != CH_COMPILE_OK ||
        lower_node(c, consequent, &cons_node, scope) != CH_COMPILE_OK) {
        ch_ir_free(test_node);
        ch_ir_free(cons_node);
        return CH_COMPILE_ERROR;
    }
    if (has_alt && lower_node(c, alternate, &alt_node, scope) != CH_COMPILE_OK) {
        ch_ir_free(test_node);
        ch_ir_free(cons_node);
        ch_ir_free(alt_node);
        return CH_COMPILE_ERROR;
...
static int push_lambda_params(ChLowerScope *child, ChValue params) {
    child->count = 0;
    ChValue p = params;
    while (ch_is_pair(p)) {
        ChValue name = ch_car(p);
        if (!ch_is_symbol(name)) {
            return -1;
        }
        if (child->count >= CH_IR_BOUND_MAX) {
            return -1;
        }
        child->names[child->count++] = ch_as_symbol(name)->name;
        p = ch_cdr(p);
    }
    if (ch_is_symbol(p)) {
        if (child->count >= CH_IR_BOUND_MAX) {
        ...
        child->names[child->count++] = ch_as_symbol(p)->name;
    } else if (!ch_is_nil(p)) {
        return -1;
    }
    return 0;
...
static ChCompileStatus lower_lambda(ChCompiler *c, ChValue expr, ChValue args, ChIrNode **out,
    ...
    ChLowerScope child;
    child.parent = scope;
    if (push_lambda_params(&child, params) != 0) {
        return make_raw(expr, out);
    }

    ChIrNode **body_nodes = NULL;
    size_t body_count = 0;
    if (lower_list_nodes(c, body, &body_nodes, &body_count, &child) != CH_COMPILE_OK) {
        return make_raw(expr, out);
    }
...
static ChCompileStatus lower_set(ChCompiler *c, ChValue expr, ChValue args, ChIrNode **out,
    ...
    if (lower_node(c, ch_car(ch_cdr(args)), &value_node, scope) != CH_COMPILE_OK) {
        return CH_COMPILE_ERROR;
    }
...
static ChCompileStatus lower_define(ChCompiler *c, ChValue expr, ChValue args, ChIrNode **out,
    ...
    ChIrNode *value_node = NULL;
    if (lower_node(c, ch_car(rest), &value_node, scope) != CH_COMPILE_OK) {
        return CH_COMPILE_ERROR;
    }
...
static ChCompileStatus lower_define_syntax(ChValue expr, ChIrNode **out) {
    /* Preserve the source form so compile_define_syntax can register macros. */
    return make_raw(expr, out);
}

static ChCompileStatus lower_variadic_seq(ChCompiler *c, ChValue expr, ChValue args, ChIrKind kind,
                                          ChIrNode **out, ChLowerScope *scope) {
    ChIrNode **items = NULL;
    size_t count = 0;
    if (lower_list_nodes(c, args, &items, &count, scope) != CH_COMPILE_OK) {
        return make_raw(expr, out);
    }
...
static ChCompileStatus lower_call_like(ChCompiler *c, ChValue expr, ChValue callee_value, ChValue args,
    ...
    size_t arg_count = 0;
    if (lower_list_nodes(c, args, &arg_nodes, &arg_count, scope) != CH_COMPILE_OK) {
        return make_raw(expr, out);
    }

    if (ch_is_symbol(callee_value) && !head_is_shadowed(callee_value, scope)) {
    ...
    if (lower_node(c, callee_value, &callee, scope) != CH_COMPILE_OK) {
...
static ChCompileStatus lower_node(ChCompiler *c, ChValue expr, ChIrNode **out,
    ...
    bool shadowed = head_is_shadowed(head, scope);

    if (!shadowed && is_symbol_named(head, "quote")) {
        return lower_quote(expr, args, out);
    }
    if (!shadowed && is_symbol_named(head, "if")) {
        return lower_if(c, expr, args, out, scope);
    }
    if (!shadowed && is_symbol_named(head, "begin")) {
        return lower_variadic_seq(c, expr, args, CH_IR_SEQ, out, scope);
    }
    if (!shadowed && is_symbol_named(head, "lambda")) {
        return lower_lambda(c, expr, args, out, scope);
    }
    if (!shadowed && is_symbol_named(head, "set!")) {
        return lower_set(c, expr, args, out, scope);
    }
    if (!shadowed && is_symbol_named(head, "define")) {
        return lower_define(c, expr, args, out, scope);
    }
    if (!shadowed && is_symbol_named(head, "define-syntax")) {
        return lower_define_syntax(expr, out);
    }
    if (!shadowed && is_symbol_named(head, "and")) {
        return lower_variadic_seq(c, expr, args, CH_IR_AND, out, scope);
    }
    if (!shadowed && is_symbol_named(head, "or")) {
        return lower_variadic_seq(c, expr, args, CH_IR_OR, out, scope);
    }
    if (!shadowed && is_derived_special_form(head)) {
        return make_raw(expr, out);
    }
    return lower_call_like(c, expr, head, args, out, scope);
}
...
ChCompileStatus ch_ir_lower(ChCompiler *c, ChValue expr, ChIrNode **out_root) {
    ...
    *out_root = NULL;
    return lower_node(c, expr, out_root, NULL);
}
