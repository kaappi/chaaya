(define-library (srfi 35)
  ...
  (begin
    ...
    (define &serious
      (really-make-condition-type '&serious &condition '() '()))

    (define &message
      (really-make-condition-type '&message &condition '(message) '(message)))

    (define &error
      (really-make-condition-type '&error &serious '() '()))
    ...
    (define (message-condition? thing)
      (and (condition? thing) (condition-has-type? thing &message)))
    (define (condition-message condition)
      (condition-ref (extract-condition condition &message) 'message))
    (define (serious-condition? thing)
      (and (condition? thing) (condition-has-type? thing &serious)))
    (define (error? thing)
      (and (condition? thing) (condition-has-type? thing &error)))))
